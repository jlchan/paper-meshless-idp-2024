...
function [points, H, H_min, A, L, E_x, E_y, S_x, S_y, Q_x, Q_y, phi_x, phi_y, normal_x, normal_y, elapsedTimeVec] = matrix_generator_optimized(a, b, h, k, r_supp, center_supp, alpha, n_boundary,n_boundary_supp, n_x, n_y, scale_x, scale_y) 
    elapsedTimeVec = [];
    tic;
    ...
    tic;
    %minspan adjacency
    %A = adjacency_matrix_min_span(points, D, a, b, h, k, alpha, n_x, n_y);
    
    %degree 1/degree 2 delauntay triangulation adjacency
    %A = adjacency_matrix_triangulation(points, a, b, h, k, alpha, n_x, n_y, n_boundary, n_boundary_supp);
    %elapsedTimeVec(4) = toc;
    
    %Euclidean radius adjacency
    A = adjacency_matrix_KT(points, a, b, h, k, alpha, n_x, n_y, n_boundary, n_boundary_supp, scale_x, scale_y);
    elapsedTimeVec(4) = toc;
...
function A = adjacency_matrix_min_span(points, dist_matrix, a, b, h, k, alpha, n_x, n_y)
    N = size(points, 1);
    A = zeros(N);a
    G = graph(dist_matrix);
...
function A = adjacency_matrix_triangulation(points, a, b, h, k, alpha, n_x, n_y, n_boundary, n_boundary_supp)
    N = size(points, 1);
...
function A = adjacency_matrix_KT(points, a, b, h, k, alpha, n_x, n_y, n_boundary, n_boundary_supp, scale_x, scale_y)
    N = size(points, 1);
